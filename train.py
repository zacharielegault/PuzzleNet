...
from nntools.dataset import ImageDataset, Composition, random_split
from nntools.utils import Config
from pytorch_lightning import Trainer, seed_everything
from pytorch_lightning.loggers import WandbLogger
from torch.utils.data import DataLoader
from callbacks import LogPredictionSamplesCallback
from network import PuzzleNet
...
def main():
    ...
    wandb_logger = WandbLogger(project='PuzzleNet',
                               config={**config.tracked_params},
                               )

    dataset = ImageDataset('/home/tmp/clpla/data/imageNet/CLS-LOC/train/',
    ...
    train_datloader = DataLoader(train_set, batch_size=config['data']['batch_size'], shuffle=True,
                                 num_workers=8, pin_memory=True, prefetch_factor=1, persistent_workers=True)

    val_datloader = DataLoader(val_set, batch_size=config['data']['batch_size'], shuffle=False, persistent_workers=True,
                               num_workers=8, pin_memory=True, prefetch_factor=1)
    ...
    trainer = Trainer(
        ...
        enable_checkpointing=True, **config['trainer'])
